# First steps
https://github.com/CzaxStudio/SharpKit.git
mkdir Myproject
cd Myproject 

# Dotnet CLI
dotnet add package SharpKit.Offensive --version 1.0.0
dotnet add package SharpKit.Offensive --version 1.0.5
dotnet add package SharpKit.Offensive --version 1.1.0
# Download using PMC
NuGet\Install-Package SharpKit.Offensive -Version 1.0.0
NuGet\Install-Package SharpKit.Offensive -Version 1.0.5
NuGet\Install-Package SharpKit.Offensive -Version 1.1.0
# Download using Packet CLI
paket add SharpKit.Offensive --version 1.0.0
paket add SharpKit.Offensive --version 1.0.5
paket add SharpKit.Offensive --version 1.1.0
# run using ./download.ps1
# Please do Follow me
